...
class SimpleLatinSquare<V> implements LatinSquare<V> {	
	...
	@Override
	public @NonNull Map<String, Cell<V>> cells() {
		return cellsView;
	}
	...
	private static <V> Map<Integer, Symbol<V>> validateSymbols(Set<Symbol<V>> symbols, int size) {
		requireNonNull(symbols, "symbols must not be null.");
		requireGreaterThanOrEqualTo(size, symbols.size(), "You must provide at least " + size + " symbols.");
